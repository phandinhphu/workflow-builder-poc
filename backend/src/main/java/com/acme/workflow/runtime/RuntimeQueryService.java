...
public class RuntimeQueryService {
    ...
    public List<Map<String,Object>>instances(String workflowId,String status,String search){
        permissions.require(current.id(), "INSTANCE_VIEW", null);
        String q=search==null?"":search.toLowerCase(Locale.ROOT);
        List<WorkflowInstanceEntity>source=workflowId==null||workflowId.isBlank()?instances.findAllByOrderByStartedAtDesc():instances.findByWorkflowIdOrderByStartedAtDesc(workflowId);
        return source.stream().filter(i->status==null||status.isBlank()||status.equals(i.status)).filter(i->q.isBlank()||i.requestCode.toLowerCase(Locale.ROOT).contains(q)||users.findById(i.creatorId).map(u->u.displayName.toLowerCase(Locale.ROOT).contains(q)).orElse(false)).map(this::summary).toList();
    }
    ...
    public Map<String,Object>instance(String id){
        WorkflowInstanceEntity i=instances.findById(id).or(()->instances.findByRequestCode(id)).orElseThrow(()->ApiException.notFound("Không tìm thấy instance "+id));
        String actor = current.id();
        if (!permissions.has(actor, "INSTANCE_VIEW", null) && !actor.equals(i.creatorId)) {
            throw ApiException.forbidden("Thiếu quyền INSTANCE_VIEW để xem chi tiết instance");
        }
