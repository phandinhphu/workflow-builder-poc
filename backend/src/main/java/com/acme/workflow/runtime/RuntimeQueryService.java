...
import com.acme.workflow.auth.*;import com.acme.workflow.common.*;import com.acme.workflow.identity.repository.*;import com.acme.workflow.runtime.domain.*;import com.acme.workflow.runtime.repository.*;import com.acme.workflow.workflow.repository.*;
import org.springframework.stereotype.Service;import java.time.Instant;import java.util.*;

@Service
public class RuntimeQueryService {
    private static final List<String>OPEN=List.of("PENDING","CLAIMED","IN_PROGRESS");
    private final WorkflowInstanceRepository instances;private final ParticipantExecutionRepository participants;private final NodeExecutionRepository executions;private final WorkflowTaskRepository tasks;private final TaskCandidateUserRepository candidates;private final EvaluationResultRepository evaluations;private final RuntimeEventRepository events;private final WorkflowDefinitionRepository workflows;private final WorkflowVersionRepository versions;private final HrmUserRepository users;private final OrganizationUnitRepository organizations;private final Jsons jsons;private final RuntimeEngineService engine;private final CurrentUserService current;private final PermissionService permissions;
    public RuntimeQueryService(WorkflowInstanceRepository instances,ParticipantExecutionRepository participants,NodeExecutionRepository executions,WorkflowTaskRepository tasks,TaskCandidateUserRepository candidates,EvaluationResultRepository evaluations,RuntimeEventRepository events,WorkflowDefinitionRepository workflows,WorkflowVersionRepository versions,HrmUserRepository users,OrganizationUnitRepository organizations,Jsons jsons,RuntimeEngineService engine,CurrentUserService current,PermissionService permissions){this.instances=instances;this.participants=participants;this.executions=executions;this.tasks=tasks;this.candidates=candidates;this.evaluations=evaluations;this.events=events;this.workflows=workflows;this.versions=versions;this.users=users;this.organizations=organizations;this.jsons=jsons;this.engine=engine;this.current=current;this.permissions=permissions;}
