 ...
 *
 * In the new Decoupled Binding Architecture (Form – Workflow – Ticket
 * Category):
 * - All workflow activation flows through: User submits Ticket -> TicketService
 * -> startWithExecutable()
 * - The old trigger types (manual, form, webhook, schedule) and their
 * enforcement logic
 * have been removed as they violate the Decoupling Principle.
 * - Background Cron Scheduler has been removed: no workflow should
 * self-activate via schedule
 * within the Ticket Platform model.
 *
 * This service is kept as a thin facade for direct API invocations (e.g.,
 * testing or admin tools).
 */
@Service
public class WorkflowTriggerService {
    ...
    public WorkflowTriggerService(WorkflowDefinitionRepository workflows, RuntimeEngineService engine) {
    ...
    // form() - form trigger type no longer exists; Form is bound at Ticket Category
    // level
    // webhook() - webhook trigger type removed; no HMAC verification in workflow
    // definition
    //
    // @Scheduled scheduled() - Background cron scheduler removed.
    // In the new Ticket Platform model, all workflow instances are initiated by
    // user actions
