...
public class RuntimeController {
    ...
    private final RuntimeQueryService query;

    public RuntimeController(RuntimeEngineService engine, RuntimeQueryService query) {
        this.engine = engine;
        this.query = query;
    }

    // New architecture: All ticket-based workflow activation goes through TicketService.
    // This endpoint remains for admin/testing direct invocation without trigger-type enforcement.
    @PostMapping("/workflows/{workflowId}/instances")
    Map<String, Object> start(@PathVariable String workflowId,
                              @RequestBody(required = false) ObjectNode body) {
        return engine.start(workflowId, body == null ? new com.fasterxml.jackson.databind.ObjectMapper().createObjectNode() : body);
    }
