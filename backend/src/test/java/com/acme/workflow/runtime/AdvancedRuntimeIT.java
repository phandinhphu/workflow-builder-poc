...
class AdvancedRuntimeIT {
    ...
    private ObjectNode linear(String id,String nodeType,ObjectNode config,String port){ObjectNode workflow=jsons.object();workflow.put("id",id).put("name",id).put("type","Assignment").put("ownerId","U000").put("draftVersion","1.0");workflow.putObject("trigger").put("type","manual").putObject("config");workflow.putArray("variables").addObject().put("key","employeeId").put("dataType","STRING").put("mutationPolicy","MUTABLE");var nodes=workflow.putArray("nodes");node(nodes.addObject(),"start","START",jsons.object());node(nodes.addObject(),"work",nodeType,config);node(nodes.addObject(),"end","END",jsons.object());var edges=workflow.putArray("connections");edge(edges.addObject(),"c1","start","SUCCESS","work");edge(edges.addObject(),"c2","work",port,"end");workflow.putObject("settings").put("maxIterations",10);return workflow;}
